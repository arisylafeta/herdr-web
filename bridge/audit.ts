...
export class AuditLog {
  ...
  record(entry: AuditEntry): void {
    ...
    try {
      const write = Promise.resolve(this.append(`${line}\n`)).catch((err) => {
        console.warn(`[audit] write failed: ${(err as Error).message}`);
      });
      this.pending.add(write);
      void write.then(() => this.pending.delete(write));
    } catch (err) {
  ...
  /** Wait until all appends accepted before this call have settled (failures are already swallowed). */
