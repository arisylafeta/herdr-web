...
import { acceptsGzip, computeEtag, gzipJsonResponse, notModified } from "./http-cache.ts";
...
describe("gzipJsonResponse", () => {
  ...
  test("respects gzip quality values and coding token boundaries", () => {
    expect(acceptsGzip("gzip;q=0, br")).toBe(false);
    expect(acceptsGzip("br, gzip;q=0.4")).toBe(true);
    expect(acceptsGzip("xgzip, br")).toBe(false);
    expect(acceptsGzip("GZIP")).toBe(true);
    expect(acceptsGzip("*;q=0.5")).toBe(true);
    expect(acceptsGzip("gzip;q=banana")).toBe(false);
    const response = gzipJsonResponse({ text: "x".repeat(300) }, "gzip;q=0");
