...
describe("herdTagFor", () => {
  test("the primary keeps the bare collie:herd tag", () => {
    expect(herdTagFor(true, "default")).toBe("herdr-control:herd");
  });
  test("a non-primary session tags collie:herd:<name>", () => {
    expect(herdTagFor(false, "collie-demo")).toBe("herdr-control:herd:collie-demo");
  });
});
...
function makeRegistry(opts: {
  ...
  disposeGates?: Record<string, Promise<void>>;
} = {}): Harness {
  ...
  const factory: SessionFactory = (name, _socketPath, _isPrimary) => {
    ...
    const fake = new FakeSession(s.bridge, s.agents, opts.disposeGates?.[name]);
    fakes.set(name, fake);
    return fake.parts();
  };
...
describe("SessionRegistry — refresh() lifecycle", () => {
  ...
  test("overlapping refresh calls dispose a vanished runtime only once", async () => {
    let releaseDisposal!: () => void;
    const disposalGate = new Promise<void>((resolve) => { releaseDisposal = resolve; });
    const h = makeRegistry({
      sessionDirs: ["demo"],
      present: ["/cfg/herdr/herdr.sock", "/cfg/herdr/sessions/demo/herdr.sock"],
      disposeGates: { demo: disposalGate },
    });
    await h.registry.refresh();
    const demo = h.fakes.get("demo")!;
    h.setDirs([]);
    ...
    const first = h.registry.refresh();
    await Promise.resolve();
    const second = h.registry.refresh();
    releaseDisposal();
  ...
  test("process-restart disposal preserves delivered notification slots", async () => {
    const h = makeRegistry();
    const primaryFake = h.fakes.get("default")!;
    await h.registry.disposeAll({ clearNotifications: false });
    expect(primaryFake.disposed).toEqual({ engine: 1, poker: 1, notifications: 0 });
  });

  test("restart can preserve only the stable primary slot and clear named-session slots", async () => {
    const h = makeRegistry({
      sessionDirs: ["demo"],
      present: ["/cfg/herdr/herdr.sock", "/cfg/herdr/sessions/demo/herdr.sock"],
    });
    ...
    await h.registry.disposeAll({ clearNotifications: (runtime) => !runtime.isPrimary });
