...
// Global events that change what the navigator renders. Pane output uses a selected-pane
// `events.wait` loop instead, because Herdr intentionally does not expose raw output as a global
// subscription filter. We deliberately DROP layout.*, worktree.*, pane.scroll_changed and
// pane.output_matched. Also NO
