...
// The upstream release check is bounded — a hung request must never wedge the monitor's timer.
...
/** The newest release among published release `tags`, as a dotted `X.Y.Z` (leading `v` stripped to match
...
export function shouldNotify(a: {
  ...
  return a.lastNotified === null || compareSemver(a.latest, a.lastNotified) > 0;
}
...
/** The GitHub release page for a published version, e.g. `…/releases/tag/v0.12.0`. */
export function githubReleaseUrl(repo: string, version: string): string {
  return `https://github.com/${repo}/releases/tag/v${version}`;
}

/** Extract stable, published release tags from the untrusted GitHub Releases response. */
export function publishedReleaseTags(value: unknown): string[] {
...
/** Anonymous HTTPS fetch of a repo's published stable GitHub Release tags. Throws on a non-OK
 *  response or timeout so the caller keeps its previous result and retries next tick. */
export function githubReleasesFetcher(repo: string): () => Promise<string[]> {
  const url = `https://api.github.com/repos/${repo}/releases?per_page=100`;
  return async () => {
    const res = await fetch(url, {
      headers: { accept: "application/vnd.github+json", "user-agent": "herdr-control-update-check" },
      signal: AbortSignal.timeout(RELEASES_TIMEOUT_MS),
    });
    if (!res.ok) throw new Error(`github releases: HTTP ${res.status}`);
    return publishedReleaseTags(await res.json());
  };
}
...
export class UpdateStateStore {
  ...
  async setLastNotified(version: string | null): Promise<void> {
...
export interface UpdateStore {
  lastNotified(): string | null;
  setLastNotified(version: string | null): Promise<void>;
}
...
export interface UpdateMonitorDeps {
  ...
  startupStamp: string;
  /** Fetch the upstream published stable release tag names (throws on failure — fail-soft). */
  fetchReleaseTags: () => Promise<string[]>;
  ...
  /** Retract the update notification slot. */
  clear: () => void;
}
...
export class UpdateMonitor {
  ...
  private lastCheckSucceeded: boolean | null = null;
  private notificationPolicyGeneration = 0;
  ...
  /**
   * One release-check cycle: fetch published releases, recompute `latest`, and fire an at-most-once push for a new
  ...
  private async runCheck(): Promise<void> {
    ...
    let tags: string[];
    try {
      tags = await this.deps.fetchReleaseTags();
    } catch {
      this.lastCheckSucceeded = false;
      return; // network / timeout — keep prior state, retry next tick
    }
    this.lastCheckSucceeded = true;
    this.latest = latestReleaseTag(tags);
    this.checkedAt = this.deps.now();

    const { current, store } = this.deps;
    const releaseAvailable = this.latest !== null && compareSemver(this.latest, current) > 0;
    if (!this.deps.updatesEnabled() || !releaseAvailable) {
      this.deps.clear();
      return;
    }
    if (
      ...
      const previousMarker = store.lastNotified();
      const policyGeneration = this.notificationPolicyGeneration;
      try {
        await store.setLastNotified(this.latest);
      } catch (error) {
        console.warn(
          `[update] could not persist notification marker: ${error instanceof Error ? error.message : String(error)}`,
        );
        return;
      }
      if (
        policyGeneration !== this.notificationPolicyGeneration ||
        !this.deps.updatesEnabled()
      ) {
        try {
          await store.setLastNotified(previousMarker);
        } catch (error) {
          console.warn(
            `[update] could not restore notification marker after opt-out: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
        this.deps.clear();
        return;
  ...
  /** Clear a stale update slot after startup if this process caught up, checks are off, or prefs deny it. */
  reconcileStartupNotification(): void {
    const lastNotified = this.deps.store.lastNotified();
    if (
      !this.deps.enabled ||
      !this.deps.updatesEnabled() ||
      (lastNotified !== null && compareSemver(this.deps.current, lastNotified) >= 0)
    ) {
      this.deps.clear();
    }
  }

  /** Explicit preference-driven retraction; safe even when no update slot exists. */
  clearNotification(): void {
    this.notificationPolicyGeneration++;
    this.deps.clear();
  }

  /** Invalidate an in-flight render, reset its durable marker, then enqueue slot retraction. */
  async prepareShutdown(): Promise<void> {
    this.notificationPolicyGeneration++;
    if (this.inFlight) await this.inFlight;
    try {
      await this.deps.store.setLastNotified(null);
    } catch (error) {
      console.warn(
        `[update] could not reset notification marker during shutdown: ${error instanceof Error ? error.message : String(error)}`,
      );
    } finally {
      // Retraction is independent of persistence: a read-only/full state directory must not leave
      // the update slot orphaned after a clean stop or uninstall.
      this.deps.clear();
    }
  }
  ...
  status(): UpdateStatus {
    ...
    return {
      ...
      lastCheckSucceeded: this.lastCheckSucceeded,
      current,
      latest: this.latest,
