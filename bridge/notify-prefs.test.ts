...
describe("NotifyPrefsStore", () => {
  ...
  test("concurrent saves serialize and persist the final merged preferences", async () => {
    const cfg = await tempCfg();
    const store = new NotifyPrefsStore(cfg);
  ...
  test("concurrent writes report previous state from inside the serialization chain", async () => {
    const store = new NotifyPrefsStore(await tempCfg());

    const [disabled, reenabled] = await Promise.all([
      store.setWithPrevious({ blocked: false }),
      store.setWithPrevious({ blocked: true }),
    ]);
  ...
  test("load rejects an invalid persisted file instead of enabling defaults", async () => {
    const cfg = await tempCfg();
    await writeFile(join(cfg.stateDir, "notify-prefs.json"), "not json");
    const store = new NotifyPrefsStore(cfg);
    await expect(store.load()).rejects.toBeDefined();
  });

  test("a failed persistence attempt leaves live preferences unchanged", async () => {
    const store = new NotifyPrefsStore({
      ...loadConfig(),
      stateDir: "/dev/null/herdr-control",
    });
    await expect(store.set({ blocked: false })).rejects.toBeDefined();
    expect(store.current()).toEqual(DEFAULT_NOTIFY_PREFS);
  });
});
